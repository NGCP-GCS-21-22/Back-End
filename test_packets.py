...
class XBeeReceiver:
	...
	def receive_packets(self, packet):
		...
		if self.packet_counter == 0:
			...
			self.packet_counter = struct.unpack("I", packet.data[:4])[0] -1 
			data = packet.data[4:]
...
def main():
	...
	while True:
		...
		try:
			...
			if gcs_cmd.perform_state == StatesMAC.LOITER:
				print("Loiter at the current altitude in a circle")
			elif gcs_cmd.perform_state == StatesMAC.TAKEOFF:
				print("Switch to manual controls and takeoff")
			elif gcs_cmd.perform_state == StatesMAC.LOCATE_HIKER:
				print("Radio search for the hiker")
			elif gcs_cmd.perform_state == StatesMAC.ERU_DROP:
				print("Payload drop")
			elif gcs_cmd.perform_state == StatesMAC.LANDING:
		...
		if cmd == "y":
			...
			data = ToGCS(50.0, 80.0, Orientation(20.0, 120.0, 270.0), LatLng(35.2, 43.5), 0.90, True, 3, False, LatLng(21.4, 8.0), 4, True, True)
